...
Options::Options()
    : comparator(BytewiseComparator()),
     ...
     // clean_write_buffer_size(12),
      clean_threshold(256 * 1024 * 1024),//vlog文件垃圾记录条数达到多少条时开始进行垃圾回收
     // clean_threshold(1*124 * 1024),
      min_clean_threshold(clean_threshold/5),//zc vlog进行清理时，只有文件失效数据大小达到min_clean_threshold才会被清理
      ...
      max_vlog_size(512*1024*1024){//vlog文件大小上限值
